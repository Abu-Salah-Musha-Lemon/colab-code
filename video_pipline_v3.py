...
def resolve_single_asset(label, folder, allowed_exts, ext_display):
    ...
    Resolve exactly ONE asset file.
      ...
      Multiple → user picks one, extras deleted
    """
    os.makedirs(folder, exist_ok=True)
        ...
        try:
            ...
            if 0 <= idx < len(files):
                ...
                for f in files:
                    if f != chosen:
                        os.remove(f)
                        print(f"     🗑️  Deleted extra: {os.path.basename(f)}")
                print(f"  ✅ {label:22s} → {chosen}")
                return chosen
...
def check_all_assets(settings):
    ...
    asset_checks = [
        ...
        ('marquee',    'Marquee',           'marquee_folder',    MEDIA_EXT,  'PNG/JPG/MP4/MOV'),
        ('overlay1',   'Overlay 1',         'overlay1_folder',   MEDIA_EXT,  'PNG/JPG/MP4/MOV'),
        ('overlay2',   'Overlay 2',         'overlay2_folder',   MEDIA_EXT,  'PNG/JPG/MP4/MOV'),
        ('bottom',     'Bottom button',     'bottom_folder',     MEDIA_EXT,  'PNG/JPG/MP4/MOV'),
...
class OverlayReader:
    """Unified reader for image or video overlays. Videos loop automatically."""
